...
export default function CellDetail(props) {
  ...
  const { gridCellId = 1 } = useParams();
  const pageParam = useParams()?.type ?? "claim";
    ...
    try {
      ...
      let auctionResult = await connector
        ...
        .then(async (result) => {
          ...
          extendedResult.bidPrice = FormatBidPrice(
            result,
            extendedResult.startingPrice,
            extendedResult.endingPrice
          );
          extendedResult.currentPrice = FormatPrice(
          ...
          extendedResult.startingPrice = FormatPrice(
            ethers.utils.formatEther(extendedResult.startingPrice)
          );
          extendedResult.endingPrice = FormatPrice(
            ethers.utils.formatEther(extendedResult.endingPrice)
          );
  ...
  useEffect(() => {
    if (Number(gridCellId) <= 0 || isNaN(Number(gridCellId))) {
          ...
          <div className={theme}>
            <div className="cell-detail">
              <DetailBanner
                ...
                owner={cell?.owner}
                sellable={
                  cell?.auction?.tokenId &&
                  account.toLowerCase() !== cell?.auction?.seller.toLowerCase()
                    ? true
                    : false
                }
